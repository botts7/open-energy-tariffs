...
| U.S. EIA residential prices (`map/baseline-us.js`: retail-sales sector RES, national + per-state) | **US public domain** (EIA API ToS: attribution, no implied endorsement, no logo, no misrepresentation — all met) | US cross-validation (national in ranking, per-state in modal) | **attribution** "derived from U.S. EIA" (shown) | ⏳ wired; needs `EIA_API_KEY` to bake |
