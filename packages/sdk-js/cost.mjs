...
/** Band id per hour (weekday/weekend) + rate/name by band id (for breakdowns). */
export function bandHours(tariff) {
...
/** Detailed annual cost breakdown: per-band {kWh,cost}, supply, export credit, total. */
export function costBreakdown(tariff, usage) {
  if (!tariff || !usage) return null;
  const bh = bandHours(tariff);
  const WD = 261, WE = 104;
  const acc = {}; let energy = 0;
  ...
  for (let h = 0; h < 24; h++) { addb(bh.weekday[h], (usage.weekday[h] || 0) * WD); addb(bh.weekend[h], (usage.weekend[h] || 0) * WE); }
